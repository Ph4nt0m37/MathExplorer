...
RAYGUIAPI int GuiTabBar(Rectangle bounds, char **text, int count, int is_closable, int *active);                        // Tab Bar control, returns TAB to be closed or -1
...
RAYGUIAPI int GuiValueBox(Rectangle bounds, const char *text, long *value, long minValue, long maxValue, bool editMode); // Value Box control, updates input text with numbers
...
int GuiTabBar(Rectangle bounds, char **text, int count, int is_closable, int *active)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < GetScreenWidth())
        {
            ...
            if (is_closable && CheckCollisionPointRec(GUI_POINTER_POSITION, tabBounds) && IsMouseButtonPressed(MOUSE_MIDDLE_BUTTON)) result = i;
...
int GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    result = GuiValueBox(valueBoxBounds, NULL, &tempValue, (long) minValue, (long) maxValue, editMode);
...
int GuiValueBox(Rectangle bounds, const char *text, long *value, long minValue, long maxValue, bool editMode)
{
    ...
    snprintf(textValue, RAYGUI_VALUEBOX_MAX_CHARS + 1, "%li", *value);
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        if (editMode)
        {
            ...
            if (valueHasChanged) *value = atol(textValue);
